...
function createTree(p_div,p_backColor,p_contextMenu,p_options) {
	...
	var tree = {
		...
		getToggleElement: function(p_node) {
			if (p_node==undefined || p_node==null || p_node.elementLi==undefined || p_node.elementLi==null)
				return null;

			var childNodes = p_node.elementLi.childNodes;
			for (var i=0; i<childNodes.length; i++) {
				if (hasClassName(childNodes[i],'exp_col'))
					return childNodes[i];
			}

			return null;
		},
		setToggleState: function(p_element,p_node) {
			if (p_element==undefined || p_element==null || p_node==undefined || p_node==null)
				return;

			var hasChildren = p_node.childNodes.length>0;
			var expanded = hasChildren ? p_node.expanded===true : true;

			p_element.id = this.getToggleId(p_node,expanded);
			p_element.style.visibility = hasChildren ? "visible" : "hidden";
			setClassName(p_element,'aimara-toggle-expanded',hasChildren && p_node.expanded===true);
			setClassName(p_element,'aimara-toggle-collapsed',hasChildren && p_node.expanded!==true);
		},
		getNodeLabelElement: function(p_node) {
			...
			for (var i=0; i<spans.length; i++) {
				if (hasClassName(spans[i],'node') || hasClassName(spans[i],'node_selected'))
					return spans[i];
			}
			...
			if (this.rendered) {
				if (p_parentNode==undefined) {
					...
					if (v_ul!=null && !parentHadChildren) {
						var v_toggle = this.getToggleElement(p_parentNode);

						if (p_parentNode.expanded) {
							this.setChildrenVisibility(v_ul,true);
							this.setToggleState(v_toggle,p_parentNode);
						}
						else {
		...
		drawNode: function(p_ulElement,p_node) {
			...
			if (p_node.childNodes.length == 0) {
				v_exp_col = createToggleElement(this.getToggleId(p_node,true));
				this.setToggleState(v_exp_col,p_node);
			}
			else {
				v_li.setAttribute('aria-expanded',p_node.expanded ? 'true' : 'false');
				v_exp_col = createToggleElement(this.getToggleId(p_node,p_node.expanded));
				this.setToggleState(v_exp_col,p_node);
			}
		...
		expandNode: function(p_node) {
			...
			if (p_node.childNodes.length>0 && p_node.expanded==false) {
				...
				if (p_node.elementLi!=null) {
					var toggle = this.getToggleElement(p_node);
					this.setToggleState(toggle,p_node);
		...
		removeNode: function(p_node) {
			...
			if (parentNode.childNodes.length==0 && parentNode.elementLi!=undefined && parentNode.elementLi!=null) {
				var v_toggle = this.getToggleElement(parentNode);
				this.setToggleState(v_toggle,parentNode);
			}
		...
		removeChildNodes: function(p_node) {
			...
			if (p_node.childNodes.length>0) {
				...
				if (p_node.elementLi!=null) {
					var v_toggle = this.getToggleElement(p_node);
					this.setToggleState(v_toggle,p_node);
				}
				...
				if (p_node!=undefined && p_node.contextMenu!=undefined && this.contextMenu!=undefined && this.contextMenu!=null) {
					...
					for (var i=0; i<v_menu.elements.length; i++) (function(i){
						...
						if (submenu!=undefined) {
							...
							v_span_more.appendChild(createMenuArrowElement());
...
function hasClassName(p_element,p_class) {
	if (p_element==undefined || p_element==null)
		return false;

	if (p_element.classList!=undefined)
		return p_element.classList.contains(p_class);

	return (' ' + (p_element.className || '') + ' ').indexOf(' ' + p_class + ' ')!=-1;
}
...
//Create expand/collapse toggle element
function createToggleElement(p_id) {
	var element = createSimpleElement('span',p_id,'exp_col');
	element.setAttribute('aria-hidden','true');
	return element;
}

//Create context menu submenu arrow element
function createMenuArrowElement() {
	var element = createSimpleElement('i',null,'menu_img');
	element.setAttribute('aria-hidden','true');
	return element;
}
...
if (typeof module!="undefined" && module.exports!=undefined) {
	...
	module.exports.default = createTree;
}
